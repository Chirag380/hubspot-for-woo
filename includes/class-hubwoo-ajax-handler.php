		...
		public function hubwoo_create_single_workflow() {
			...
			if ( ! empty( $_POST['name'] ) ) {
				...
				if ( ! empty( $workflow_details ) ) {
					...
					echo wp_json_encode( $original_response );
					wp_die();
				}
			}
		}
				...
				if ( ! empty( $process ) ) {
					...
					if ( 'start-deal' === $process ) {
						...
						if ( $orders_needs_syncing ) {
							...
							wp_schedule_event( time(), 'mwb-hubwoo-deals-5min', 'hubwoo_deals_sync_background' );
						}
					} else {
				...
				if ( ! empty( $process ) ) {
					switch ( $process ) {
						...
						case 'start-products-sync':
							if ( ! wp_next_scheduled( 'hubwoo_products_sync_background' ) ) {
								update_option( 'hubwoo_start_product_sync', 'yes' );
								wp_schedule_event( time(), 'mwb-hubwoo-products-3min', 'hubwoo_products_sync_background' );
							}
